...
function renderTestRun() {
    ...
    container.innerHTML = currentRunState.map((test, index) => {
        ...
        // If the next item is indented deeper, this one is a category header
        ...
        return `
            <div class="test-row level-${test.level} ${isHeader ? 'is-header' : ''}">
                ...
                <div class="test-controls ${isHeader ? 'hidden' : ''}">
                    ...
                    <input type="text" placeholder="Note..." value="${test.notes || ''}" 
...
function renderEditor() {
    ...
    container.innerHTML = editingTests.map((t, i) => `
        <div class="editor-line" style="margin-left: ${t.level * 40}px">
            <button class="status-btn" onclick="move(${i}, -1)">◀</button>
            <button class="status-btn" onclick="move(${i}, 1)">▶</button>
            <input type="text" class="editor-input" value="${t.text}" oninput="editingTests[${i}].text=this.value" placeholder="Requirement/Step name...">
            <button onclick="editingTests.splice(${i},1);renderEditor()" style="border:none; background:none; cursor:pointer; color:red; padding: 5px;">✕</button>
        </div>
    `).join('');
}
...
function saveSuite() {
    ...
    if(!name) return alert("Please enter a name for this suite.");
...
function renderHistory() {
    document.getElementById('history-list').innerHTML = history.map(h => `
        <div class="card" style="margin-bottom:10px">
            ...
            <textarea readonly style="width:100%; height:60px; margin-top:10px; border:1px solid #eee; padding:5px; font-size:12px; font-family:monospace;">${h.report}</textarea>
        </div>
    `).join('');
}
